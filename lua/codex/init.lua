...
function M.setup(user_config)
  ...
  vim.api.nvim_create_user_command('CodexNew', function(opts)
    M.new_session({ title = opts.args ~= '' and opts.args or nil })
  end, {
  ...
  vim.api.nvim_create_user_command('CodexList', function()
    M.pick_session()
  end, { desc = 'List Codex sessions' })
  vim.api.nvim_create_user_command('CodexNext', function()
    M.next_session()
  end, { desc = 'Next Codex session' })
  vim.api.nvim_create_user_command('CodexPrev', function()
    M.prev_session()
  end, { desc = 'Prev Codex session' })
  vim.api.nvim_create_user_command('CodexWhere', function()
    vim.notify(source_path(), vim.log.levels.INFO)
...
local function create_session_buf()
  ...
  if config.keymaps.quit then
    map('n', config.keymaps.quit, function()
      require('codex').close()
    end)
  end
  ...
  map('n', config.keymaps.next, function()
    require('codex').next_session()
  end)
  map('n', config.keymaps.prev, function()
    require('codex').prev_session()
  end)
  map('n', config.keymaps.new, function()
    require('codex').new_session()
  end)
  map('n', config.keymaps.list, function()
    require('codex').pick_session()
  end)
  map('n', config.keymaps.close_session, function()
    local id = state.current
    if id then
      require('codex').close_session(id)
    end
  end)
...
local function ensure_cmd_present(on_ready)
  local check_cmd = type(config.cmd) == 'string' and not config.cmd:find('%s') and config.cmd
    or (type(config.cmd) == 'table' and config.cmd[1])
    or nil
